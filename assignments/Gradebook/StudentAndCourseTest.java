...
public class StudentAndCourseTest extends TestCase {
	...
	public void testCreateLegacy() {
		for (int i = 0; i < 100; i++) {
			...
			assertTrue("create baby not setting name properly",
					bb.getName().contains(s.getName()) && bb.getName().contains(ss.getName()));
			...
			assertTrue("create baby not creating ID properly",
					bb.getStudentID() == s.getStudentID() + ss.getStudentID());
	...
	// TESTING COURSE CLASS HERE . . . FEEL FREE TO WRITE YOUR OWN, BUT DON'T
	// CHANGE THIS ONE
	...
	public void testCourseInit() {
		...
		for (int i = 0; i < 20; ++i) {
			...
			assertEquals("course getRemainingSeats() not working, or seats not being set properly", s,
					c2.getRemainingSeats());
		}
	}
